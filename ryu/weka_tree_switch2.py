    ...
    def _packet_in_handler(self, ev):
        ...
        #     #print fluxo_classe
            ...
            if fluxo_classe_temp == "web" and ipv4_temp.proto == TCP_CODE:
                match = parser.OFPMatch(in_port=in_port, eth_dst=dst, ip_proto= ipv4_temp.proto, ipv4_dst = ipv4_temp.dst, tcp_dst =  tcp_temp.dst_port )
                #actions= parser.OFPActionSetQueue(queue_id=1)
            elif  fluxo_classe_temp == "dns" and ipv4_temp.proto == UDP_CODE:
                match = parser.OFPMatch(in_port=in_port, eth_dst=dst,ip_proto= ipv4_temp.proto,ipv4_dst = ipv4_temp.dst, tcp_dst =  udp_temp.dst_port  )
                #actions = parser.OFPActionSetQueue(queue_id=2)
